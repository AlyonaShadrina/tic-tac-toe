...
export class Game {
  ...
  makeMove(
    ...
    this.currentPlayerMoveIndex =
      this.currentPlayerMoveIndex === this.players.length - 1
        ? 0
        : this.currentPlayerMoveIndex + 1;
  ...
  private isSymbolRegistered(symbol: string) {
    return this.players.find(
      (exitingPlayer) => exitingPlayer.symbol === symbol,
    );
  }
}
