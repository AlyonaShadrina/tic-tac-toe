...
export class Field implements IField {
  constructor(public cells: IField['cells']) {}
  ...
  hasNSymbolsInRow(
    ...
    const existingSurroundingCoordinates =
      potentialySurroundingCoordinates.filter(
        (coordinatesStrings) => this.cells[coordinatesStrings] === symbol,
      );

    // check if next cell within provided vector has relevant symbol
    const checkPrevCellSymbol = (
      coordinate: TCoordinates,
      vector: TCoordinates,
      potentionalyWinRow: any,
    ) => {
      const prevPotentialCoordinate: TCoordinates = [
        coordinate[0] - vector[0],
        coordinate[1] - vector[1],
      ];
      const prevPotentialCoordinateString = JSON.stringify(
        prevPotentialCoordinate,
      );
    ...
    const checkNextCellSymbol = (
      coordinate: TCoordinates,
      vector: TCoordinates,
      potentionalyWinRow: any,
    ) => {
      const nextPotentialCoordinate: TCoordinates = [
        coordinate[0] + vector[0],
        coordinate[1] + vector[1],
      ];
      const nextPotentialCoordinateString = JSON.stringify(
    ...
    for (let i = 0; i < existingSurroundingCoordinates.length; i++) {
      ...
      const vector: TCoordinates = [
        currentCoordinatesX - coordinates[0],
        currentCoordinatesY - coordinates[1],
      ];

      // check next coordinates in provided vector
      const nextPotentialCoordinate: TCoordinates = [
        currentCoordinatesX + vector[0],
        currentCoordinatesY + vector[1],
      ];
      const nextPotentialCoordinateString = JSON.stringify(
        nextPotentialCoordinate,
      );
